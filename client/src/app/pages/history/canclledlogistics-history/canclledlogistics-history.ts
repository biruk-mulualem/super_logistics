...
@Component({
  selector: 'app-canclledlogistics-history',
 imports: [Header, Sidebar, ReusableTable],
  templateUrl: './canclledlogistics-history.html',
  styleUrl: './canclledlogistics-history.css'
})

// export class CanclledlogisticsHistory implements OnInit {
export class CanclledlogisticsHistory {
 tableHeaders = [
    { label: 'Id', key: 'id' },
...
ngOnInit() {
  ...
  this.tableData = data.tableData || [];
    ...
    async loadFollowups() {
    try {
      const data = await firstValueFrom(this.logisticsService.getLogisticsStatus0Data());
      if (!data) { this.tableData = []; return; }

      const tableWithDetails = await Promise.all(
        data.map(async row => {
          const [items] = await Promise.all([
            firstValueFrom(this.logisticsService.getLogisticsItemsDetailStatus0Data(row.transactionId))
