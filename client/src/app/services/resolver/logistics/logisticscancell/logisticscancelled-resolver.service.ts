...
export class LogisticsCancelledResolverService implements Resolve<any> {
  constructor(
...
async resolve() {
  try {
    const logistics = await firstValueFrom(this.service.getLogisticsStatus0Data());

    const tableWithItems = await Promise.all(
      logistics.map(async row => {
        const items = await firstValueFrom(this.service.getLogisticsItemsDetailStatus0Data(row.transactionId));
        return { ...row, items };
      })
    );
