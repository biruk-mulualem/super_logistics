...
export class ReusableTable implements OnInit, OnChanges {
  ...
  private updatePageTypeAndButtons() {
    ...
    const config = this.routeConfigs.find((cfg) =>
      currentRoute.includes(cfg.match)
    );
    if (config) {
  ...
  closeModal(modal?: keyof typeof this.showModal) {
    if (modal) this.showModal[modal] = false;
      ...
      Object.keys(this.showModal).forEach(
  ...
  validateIntransitPayments(payments: any[]): string[] {
    ...
    payments.forEach((p, idx) => {
      ...
      if (!p.accountPaidFrom?.trim())
        missing.push(`Payment #${idx + 1} Account Paid From`);
  ...
  validateIntransitAddData(data: any): string[] {
    ...
    data.items.forEach((i: any, idx: number) => {
      if (!i.itemDescription?.trim())
        missing.push(`Item #${idx + 1} Description`);
      if (!i.uom?.trim()) missing.push(`Item #${idx + 1} UOM`);
      if (!i.quantity || isNaN(Number(i.quantity)))
        missing.push(`Item #${idx + 1} Quantity`);
      if (!i.unitPrice || isNaN(Number(i.unitPrice)))
        missing.push(`Item #${idx + 1} Unit Price`);
  ...
  openMainModal(row: any) {
    ...
    // ✅ Populate editable arrays for all 4 tables
    this.setEditableRows();
  ...
  validateLogisticsAddData(): string[] {
    const missing: string[] = [];
    if (!this.addData.LoadedOnfcl || isNaN(Number(this.addData.LoadedOnfcl)))
      missing.push('Loaded On FCL');
    if (!this.addData.containerType?.trim()) missing.push('Container Type');

    if (!this.addData.items || this.addData.items.length === 0)
      missing.push('At least one item');
    else {
      this.addData.items.forEach((item: any, idx: number) => {
        if (!item.transactionId?.trim())
          missing.push(`Item #${idx + 1} Transaction ID`);
        if (!item.itemDescription?.trim())
          missing.push(`Item #${idx + 1} Description`);
        if (!item.uom?.trim()) missing.push(`Item #${idx + 1} UOM`);
        if (!item.LoadedQnty || isNaN(Number(item.LoadedQnty)))
          missing.push(`Item #${idx + 1} Loaded Quantity`);
      });
    }
  ...
  setEditableRows() {
    ...
    this.rowDjbDeparted = (this.selectedRow.djbDeparted ?? []).map(
      (d: { numberOfContainer: any; remark: any; date: any }) => ({
        numberOfContainer: d.numberOfContainer,
        remark: d.remark,
        date: d.date,
      })
    );

    this.rowAakArrived = (this.selectedRow.arrivedAAk ?? []).map(
      (a: { numberOfContainer: any; remark: any; date: any }) => ({
        numberOfContainer: a.numberOfContainer,
        remark: a.remark,
        date: a.date,
      })
    );

    this.rowSdtArrived = (this.selectedRow.arrivedSDT ?? []).map(
      (s: { numberOfContainer: any; remark: any; date: any }) => ({
        numberOfContainer: s.numberOfContainer,
        remark: s.remark,
        date: s.date,
      })
    );

    this.rowsContainerReturned = (this.selectedRow.containerReturned ?? []).map(
      (c: { numberOfContainer: any; remark: any; date: any }) => ({
        numberOfContainer: c.numberOfContainer,
        remark: c.remark,
        date: c.date,
      })
    );
  ...
  rowAakArrived: { numberOfContainer: number; remark: string; date: string }[] =
    [];
  rowSdtArrived: { numberOfContainer: number; remark: string; date: string }[] =
    [];
  rowsContainerReturned: {
    numberOfContainer: number;
    remark: string;
    date: string;
  }[] = [];
  ...
  // ================== AAK Arrived ==================
  addRowAakArrived() {
    this.rowAakArrived.push({ numberOfContainer: 0, remark: '', date: '' });
  }
  ...
  // ================== SDT Arrived ==================
  addRowSdtArrived() {
    this.rowSdtArrived.push({ numberOfContainer: 0, remark: '', date: '' });
  }
  ...
  addRowContainerReturned() {
    this.rowsContainerReturned.push({
      numberOfContainer: 0,
      remark: '',
      date: '',
    });
